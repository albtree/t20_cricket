...
wpa_summarise_player_comp <- function(x){test_mbb <- x %>%
  group_by(match_id) %>%
...
batters_wpa <- test_mbb %>%
  group_by(striker, batting_team, competition, season) %>%
...
bowlers_wpa <- test_mbb %>%
  group_by(bowler, bowling_team, competition, season) %>%
...
wpa_both <- wpa_bat_only %>%
  ...
  replace(., is.na(.),0)%>%
  ungroup()%>%
  group_by(player, team, competition, season) %>%
